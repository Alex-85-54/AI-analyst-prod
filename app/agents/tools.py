...
from app.context import set_last_query_df
...
def _normalize_query_input(inp) -> str:
    """Приводит вход инструмента к строке SQL (tool-calling может передать dict)."""
...
def _clickhouse_query_with_context(query):
    """Обёртка: выполняет запрос к ClickHouse и сохраняет последний DataFrame в контекст."""
...
def _postgres_query_with_context(query):
    """Обёртка: выполняет запрос к PostgreSQL и сохраняет последний DataFrame в контекст."""
    query_str = _normalize_query_input(query)
